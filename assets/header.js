...
class HeaderComponent extends Component {
  ...
  /**
   * Minimum scroll delta (in pixels) required to trigger a direction change.
   * Prevents jitter on mobile from causing rapid show/hide toggling.
   * @constant {number}
   */
  #scrollThreshold = 5;
  ...
  #updateScrollState = () => {
    ...
    // Apply scroll threshold for scroll-up mode to prevent mobile jitter
    const isScrollingUp = scrollDelta < -this.#scrollThreshold;
    const isScrollingDown = scrollDelta > this.#scrollThreshold;
    ...
    // Within threshold deadzone: don't update lastScrollTop, allowing delta to accumulate
  };
